...
const fieldSpec = Joi.object()
  .keys({
    ...
    defaultValue: Joi
      .when('type', { is: 'timestamp', then: Joi.number().integer() })
      .when('type', { is: 'string', then: Joi.string().allow('') })
      .when('type', { is: 'integer', then: Joi.number().integer().allow('') })
...
const schemaSchema = Joi.object()
  .keys({
    ...
    nodeTypes: Joi.array().required().items(
      Joi.object().keys({
...
function lintSchema (schema) {
  const result = schemaSchema.validate(schema, { abortEarly: false })
...
function validateNodeTypesUnique (schema) {
  ...
  const dups = Object.keys(nodeTypesById).filter(function (id) {
  ...
  if (dups.length > 0) {
    dups.forEach(function (dupId) {
      const typeNames = nodeTypesById[dupId].join(', ')
      errors.push('duplicate node type IDs for types [' + typeNames + '] (id = ' + dupId + ')')
    })
  }

...
function encode (roots, schema, options) {
  const encoder = new QueryPackEncoder(schema)
...
QueryPackEncoder.prototype.encodeString = function (fieldSpec, str) {
  if (typeof str !== 'string') throw new Error('Attempt to encode non-string value as string')
...
QueryPackEncoder.prototype.encodeTimestamp = function (fieldSpec, val) {
  if (typeof val !== 'number') throw new Error('Attempt to encode non-numeric value as timestamp')
...
QueryPackEncoder.prototype.encodeInt = function (fieldSpec, val) {
  if (typeof val !== 'number') throw new Error("Attempt to encode non-numeric value '" + util.format(val) + "' as integer in field '" + fieldSpec.name + "'")
...
QueryPackEncoder.prototype.encodeDouble = function (fieldSpec, val) {
  if (typeof val !== 'number') throw new Error("Attempt to encode non-numeric value '" + util.format(val) + "' as double in field '" + fieldSpec.name + "'")
  if (this.omitDefaults && val === fieldSpec.defaultValue) return ''
  return '' + val
}

QueryPackEncoder.prototype.encodeBoolean = function (fieldSpec, val) {
  if (typeof val !== 'boolean') throw new Error("Attempt to encode non-boolean value '" + util.format(val) + "' as boolean in field '" + fieldSpec.name + "'")
  if (this.omitDefaults && val === fieldSpec.defaultValue) return ''
  return val ? 'T' : 'F'
}
...
QueryPackEncoder.prototype.encodeNodeList = function (fieldSpec, val) {
  const encoder = this
  if (!Array.isArray(val)) throw new Error("Attempt to encode non-array value '" + util.format(val) + "' as nodeList for field '" + fieldSpec.name + "'")
...
QueryPackEncoder.prototype.encodeEnum = function encodeEnum (fieldSpec, fieldVal) {
  const index = fieldSpec.values.indexOf(fieldVal)
  if (index === -1) throw new Error("Unknown value ''" + fieldVal + "'for field '" + fieldSpec.name)
...
QueryPackEncoder.prototype.encodeNode = function encodeNode (n) {
  ...
  this.children = []
  let node = ''
  const type = this.typesByName[n.type]
  if (!type) {
    throw new Error("Unrecognized type '" + n.type + "' in input JSON")
  }

  const typeId = this.typesByName[n.type].id
...
QueryPackEncoder.prototype.encode = function (roots) {
  ...
  const schemaId = encoder.schema.name + '.' + encoder.schema.version + ';'

  return schemaId + roots.map(function (node) {
    return encoder.encodeNode(node)
  }).join(';')
}

QueryPackEncoder.prototype.encodeValue = function encodeValue (fieldSpec, node, addSeperator) {
  const fieldName = fieldSpec.name
  const fieldVal = node[fieldName]
  const fieldType = fieldSpec.type

  if (fieldType === 'node') {
    if (fieldVal !== null && fieldVal.type !== fieldSpec.nodeType) {
      throw new Error("Expected node of type '" + fieldSpec.nodeType + "' saw '" + fieldVal.type + "' for field '" + fieldName + "'")
    }
  ...
  if (fieldVal === null) {
    if (!fieldSpec.nullable) {
      throw new Error("Saw null value for non-nullable field '" + fieldName + "'")
    }
  ...
  if (!method) throw new Error("Unknown field type '" + fieldType + "' in node " + util.format(node))

  let encodedVal = this[method](fieldSpec, fieldVal)
